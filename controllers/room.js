...
export const deleteGalleryPhoto = async (req, res) => {
    try {
      ...
      if (!hotel) {
        return res.status(400).json({ message: `Cannot find hotel with id ${req.params.id}` });
      }
  
      // Find the index of the image in the gallery_images array
      const imageIndex = hotel.gallery_images.findIndex(image => image === req.body.imgPath);
